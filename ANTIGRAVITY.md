...
This file serves as a durable record of the architectural consensus reached regarding OpenAgent, specifically focusing on the deployment realities for Raspberry Pi and the hybrid Python/Go/Rust split.
...
- **Solution:** Go/Rust services write data directly to the shared filesystem (`data/artifacts/`). They emit a tiny JSON payload containing only the pointer/path (`{"path": "/data/artifacts/audio_123.ogg"}`).
- Python receives this path and passes it as an argument to the next service in the workflow. 

## 4. No East-West Mesh
Go and Rust services **never** talk to each other directly. 
