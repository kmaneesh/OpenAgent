//! OTEL observability — all four pillars — for the Telegram service.
//!
//! ```text
    ...
    pub fn record(&self, data: &Value) {
        ...
        if guard.current_date != today {
            match open_metrics_file(&self.inner.logs_dir, &today) {
                ...
                Err(e) => {
                    eprintln!("telegram metrics rotate error: {e}");
                    return;
                }
            }
        }
...
fn open_metrics_file(dir: &PathBuf, date: &str) -> anyhow::Result<File> {
    let path = dir.join(format!("telegram-metrics-{date}.jsonl"));
